		...
		if err == nil {
			...
			if tt.Before(timeNow) == true {
				if valid == true {
					victory = "over"
					mu.Lock()
					BlockChain.Blocks = append(BlockChain.Blocks, block) //添加新块
					mu.Unlock()
			...
			if victory == "false" && result == true && isMined == true {
				...
				if err != nil {
					...
					if string(buf[:n]) == "Validation victory!" {
						...
						//验证失败，表示挖矿挖错了，重新开始挖矿--进入下一次循环
						fmt.Printf("挖矿结果未经过另一个节点的检验，nonce：%v\n", block.Nonce)
						fmt.Println("Validation false!")
						continue
					} else if string(buf[:n]) == "sorry i refused your mined!" {
						continue
					}
				}
				timeNow = someCompose.InitTime()
...
func main() {
	...
	serverAddress1 := "127.0.0.1:20002"
	...
	var err1 error
	for {
		conn, err1 = net.Dial("tcp", "127.0.0.1:20003")
		if err1 != nil {
			//fmt.Printf("conn:%v\n", err1)
		} else {
			wg.Add(1)
